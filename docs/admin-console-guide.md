...
Grafana와의 역할 분리:
- Admin 메트릭 메뉴: 운영자 즉각 진단(at-a-glance health). DB/Redis 의존 없이 prom-client in-memory에서 직접 산출.
- Grafana(nerdvana-grafana): 시계열 추이, 알림 규칙, 장기 보관 데이터. Prometheus가 `/metrics` 엔드포인트를 scrape하여 수집. 설정 위치: `/home/nirna/job/mcp/docs-mcp/monitoring/`.
