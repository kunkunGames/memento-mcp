...
export function startSchedulers({ globalEmbeddingWorkerRef }) {
  ...
  /** 합성 역질의 워커 시작. 설정이 꺼져 있으면 start()가 즉시 반환한다. */
  import("./memory/embedding/SyntheticQueryWorker.js")
    .then(({ getSyntheticQueryWorker }) => getSyntheticQueryWorker().start())
    .catch(err => logWarn(`[SyntheticQueryWorker] 기동 실패: ${err.message}`));
