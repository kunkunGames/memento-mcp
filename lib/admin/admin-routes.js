...
import { ADMIN_ALLOWED_ORIGINS } from "../config.js";
...
import { buildMetricsSummary }        from "./admin-metrics.js";
...
import { getSearchObservability, getPathPerformance } from "../memory/signals/SearchEventAnalyzer.js";
...
export async function handleAdminApi(req, res) {
  ...
  // 화이트리스트에 등록된 Origin만 반사한다. 미등록·서버 호출(Origin 없음)에는
  // ACAO를 설정하지 않아 브라우저 교차 출처 판독을 차단한다.
  const origin = req.headers.origin;
  if (origin && ADMIN_ALLOWED_ORIGINS.has(String(origin))) {
  ...
  if (req.method === "GET" && url.pathname === `${ADMIN_BASE}/stats`) {
    try {
      ...
      const [fragR, callR, keyR, topicR, backlogR, qualityR] = await Promise.all([
        pool.query(`SELECT COUNT(*) AS total FROM ${SCHEMA}.fragments`),
        pool.query(`SELECT COALESCE(SUM(call_count),0) AS total
                      FROM ${SCHEMA}.api_key_usage
                     WHERE usage_date = CURRENT_DATE`),
        pool.query(`SELECT COUNT(*) AS total FROM ${SCHEMA}.api_keys WHERE status='active'`),
        ...
        pool.query(`SELECT COUNT(*) FILTER (WHERE quality_verified)::int AS verified,
                           COUNT(*)::int AS total
                      FROM ${SCHEMA}.fragments
                     WHERE valid_to IS NULL`)
      ]);
      ...
      const [smResult, obsResult, qpResult, psResult, ppResult] = await Promise.allSettled([
        ...
        pool.query(`SELECT COUNT(*) AS cnt FROM ${SCHEMA}.fragments WHERE quality_verified IS NULL`),
      ...
      res.end(JSON.stringify({
        ...
        topTopics:      topicR.rows,
        quality:        { verified: qualityR.rows[0]?.verified ?? 0, total: qualityR.rows[0]?.total ?? 0 },
        ...
        queues:          {
          embeddingBacklog: backlogR.rows[0]?.count ?? 0,
  ...
  /** GET /metrics-summary */
  if (req.method === "GET" && url.pathname === `${ADMIN_BASE}/metrics-summary`) {
    try {
      const windowSec = parseInt(url.searchParams.get("windowSec") || "60", 10) || 60;
      const includeRaw = url.searchParams.get("include");
      const include    = includeRaw
        ? includeRaw.split(",").map(s => s.trim()).filter(Boolean)
        : ["cards", "tools", "errors"];

      const summary = await buildMetricsSummary({ windowSec, include });
      res.statusCode = 200;
      res.end(JSON.stringify(summary));
    } catch (err) {
      logError("[Admin] /metrics-summary error:", err);
      res.statusCode = 500;
      res.end(JSON.stringify({ error: safeErrorMessage(err) }));
    }
    return;
  ...
  if (await handleSearch(req, res, url)) return;
  if (await handleSearchEvents(req, res, url)) return;
