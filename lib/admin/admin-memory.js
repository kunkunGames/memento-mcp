...
import { keyScopeClause, keyScopeCondition, keyScopeScalar, keyScopeGroup }       from "../memory/keyScope.js";
...
export async function fetchGroupKeyIds(pool, groupId) {
  const { rows } = await pool.query(
    `SELECT key_id FROM ${SCHEMA}.api_key_group_members WHERE group_id = $1`,
    [groupId]
  );
...
export async function handleMemory(req, res, url) {
  ...
  if (subPath === "/overview")            return handleOverview(req, res, url);
  if (subPath === "/search-events")       return handleMemorySearchEvents(req, res, url);
  if (subPath.startsWith("/fragments/"))  return handleFragmentDetail(req, res, url, subPath);
  if (subPath === "/fragments")           return handleFragmentList(req, res, url);
  if (subPath === "/anomalies")           return handleAnomalies(req, res, url);
  if (subPath === "/graph")               return handleGraph(req, res, url);

  return false;
}

/** 조회 라우트 처리기. 응답을 직접 쓴다. */
async function handleOverview(req, res, _url) {
  try {
    const pool = getPrimaryPool();

    const [totalR, typeR, topicR, pendingR, supersededR, recentR] = await Promise.all([
      pool.query(`SELECT COUNT(*)::int AS total FROM ${SCHEMA}.fragments`),
...
/** 조회 라우트 처리기. 응답을 직접 쓴다. */
async function handleMemorySearchEvents(req, res, url) {
  try {
    const pool   = getPrimaryPool();
...
/** 조회 라우트 처리기. 응답을 직접 쓴다. */
async function handleFragmentDetail(req, res, url, subPath) {
  ...
  try {
    ...
    if (groupId) {
      ...
      conditions.push(keyScopeCondition(params, "key_id", memberKeyIds));
    } else if (keyId) {
      conditions.push(keyScopeCondition(params, "key_id", keyId));
    }
 ...
 * 파편 목록 질의의 단순 필터 사상표.
 *
 * 조회 인자마다 컬럼과 비교 연산이 다를 뿐 조립 절차는 같다. 차이를 표로 두고
 * 절차는 한 번만 쓴다. 키 격리와 그룹 조회는 별도 판정이 필요해 표 밖에 둔다.
 */
const FRAGMENT_FILTERS = [
  { param: "q",                column: "content",          op: "ILIKE", like: true },
  { param: "topic",            column: "topic",            op: "ILIKE", like: true },
  { param: "type",             column: "type",             op: "=" },
  { param: "assertion_status", column: "assertion_status", op: "=" },
  { param: "ttl_tier",         column: "ttl_tier",         op: "=" },
  { param: "case_id",          column: "case_id",          op: "=" },
  { param: "created_from",     column: "created_at",       op: ">=" },
  { param: "created_to",       column: "created_at",       op: "<=" }
];

/**
 * 조회 인자에서 WHERE 조건과 바인딩을 만든다.
 *
 * @param {URL}   url
 * @param {Array} params 바인딩 배열 (in-place로 push된다)
 * @param {Object} pool  그룹 조회용 커넥션 풀
 * @returns {Promise<string[]>} 조건 문자열 배열
 */
async function buildFragmentFilters(url, params, pool) {
  const conditions = [];

  for (const spec of FRAGMENT_FILTERS) {
    const raw = url.searchParams.get(spec.param);
    if (!raw) continue;
    params.push(spec.like ? `%${escapeLike(raw)}%` : raw);
    conditions.push(`${spec.column} ${spec.op} $${params.length}`);
  }

  const isAnchorRaw = url.searchParams.get("is_anchor");
  if (isAnchorRaw === "true" || isAnchorRaw === "false") {
    params.push(isAnchorRaw === "true");
    conditions.push(`is_anchor = $${params.length}`);
  }

  const minAccess = parseInt(url.searchParams.get("min_access_count"), 10);
  if (!Number.isNaN(minAccess)) {
    params.push(minAccess);
    conditions.push(`access_count >= $${params.length}`);
  }

  /** 키 격리. 단일 키, 콤마 구분 목록, 그룹 소속 세 경로가 있다. */
  const keyId = url.searchParams.get("key_id");
  if (keyId) conditions.push(keyScopeCondition(params, "key_id", keyId));

  const listScope = parseKeyIdsScope(url);
  if (listScope.groupKeyIds) conditions.push(keyScopeCondition(params, "key_id", listScope.groupKeyIds));

  const groupId = url.searchParams.get("group_id") || null;
  if (groupId) {
    const memberKeyIds = await fetchGroupKeyIds(pool, groupId);
    /** 구성원이 없는 그룹은 결과가 없어야 한다. 조건을 빼면 전체가 보인다. */
    conditions.push(memberKeyIds.length > 0
      ? keyScopeCondition(params, "key_id", memberKeyIds)
      : "FALSE");
  }
...
/** 조회 라우트 처리기. 응답을 직접 쓴다. */
async function handleFragmentList(req, res, url) {
  try {
    const pool     = getPrimaryPool();
    ...
    const params      = [];
    const conditions  = await buildFragmentFilters(url, params, pool);
    const whereClause = conditions.length > 0 ? "WHERE " + conditions.join(" AND ") : "";

    const countParams = [...params];
    const countSql    = `SELECT COUNT(*)::int AS total FROM ${SCHEMA}.fragments ${whereClause}`;
...
/** 조회 라우트 처리기. 응답을 직접 쓴다. */
async function handleAnomalies(req, res, _url) {
  try {
    const pool = getPrimaryPool();
    ...
    const [unverifiedR, supersessionR, failedR, staleR] = await Promise.all([
      ...
      pool.query(`SELECT COUNT(DISTINCT f.id)::int AS count
                    ...
                    JOIN ${SCHEMA}.fragment_links fl ON fl.from_id = f.id
                   WHERE NOT EXISTS (
...
async function handleGraph(req, res, url) {
  try {
    ...
    if (keyId) {
      fragQuery += keyScopeScalar(fragParams, "key_id", keyId);
    } else if (groupId) {
      const memberKeyIds = await fetchGroupKeyIds(pool, groupId);
      if (memberKeyIds.length > 0) {
        fragQuery += keyScopeGroup(fragParams, "key_id", memberKeyIds);
      } else {
        fragQuery += ` AND FALSE`;
      }
    }
