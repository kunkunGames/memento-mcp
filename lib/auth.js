...
export async function validateAuthentication(req, msg) {
  /**
   * 키가 없을 때의 처리는 buildAuthDecision 한곳에서 정한다.
   *
   * 종전에는 여기서 ACCESS_KEY 부재를 무조건 통과로 처리해, 환경 변수 하나가
   * 빠지면 인터넷에 노출된 서버가 모든 도구와 master 범위를 무인증으로 열었다.
   * 무인증 운용은 MEMENTO_AUTH_DISABLED=true를 명시했을 때만 성립한다.
   */
  if (!ACCESS_KEY) {
    const decision = buildAuthDecision(ACCESS_KEY, AUTH_DISABLED, null);
    return decision.valid
      ? { valid: true, keyId: null, groupKeyIds: null }
      : { valid: false, error: decision.error ?? "access_key_required" };
  }
