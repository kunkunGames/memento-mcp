...
export class ClaimStore {
  ...
  async insert(fragment, claims, ctx = {}) {
    ...
    const sql = `
      INSERT INTO ${escapeId(SCHEMA)}.fragment_claims
  ...
  async getByFragmentId(fragmentId, keyId = null) {
    ...
    const sql = `
      SELECT id, fragment_id, key_id, subject, predicate, object,
        ...
        FROM ${escapeId(SCHEMA)}.fragment_claims
       WHERE fragment_id = $1
  ...
  async deleteByFragmentId(fragmentId, keyId = null) {
    ...
    const { rowCount } = await pool.query(
      `DELETE FROM ${escapeId(SCHEMA)}.fragment_claims
        WHERE fragment_id = $1
          AND key_id IS NOT DISTINCT FROM $2`,
      params
    );
  ...
  async findPolarityConflicts(fragmentId, keyId = null, opts = {}) {
    ...
    const sql = `
      SELECT c1.fragment_id AS f1,
             ...
             c1.predicate,
             c1.object
        FROM ${escapeId(SCHEMA)}.fragment_claims c1
        JOIN ${escapeId(SCHEMA)}.fragment_claims c2
