    ...
    try {
      ...
      const s35 = await runStage("gc_preview", async () => {
        const gcPreview = await queryWithAgentVector("system",
          `SELECT type, COUNT(*) as cnt FROM ${escapeId(SCHEMA)}.fragments
  ...
  async _transitionWithCount() {
    const before = await queryWithAgentVector("system",
      `SELECT ttl_tier, count(*)::int AS cnt
             FROM ${escapeId(SCHEMA)}.fragments GROUP BY ttl_tier`
    );
    ...
    for (const topicRow of topicsResult.rows) {
      ...
      for (const frag of fragsResult.rows) {
        ...
        for (const candidate of knnResult.rows) {
          ...
          /** 링크 이전 (from_id) */
          await queryWithAgentVector("system",
            `UPDATE ${escapeId(SCHEMA)}.fragment_links SET from_id = $1
          ...
          /** 링크 이전 (to_id) */
          await queryWithAgentVector("system",
            `UPDATE ${escapeId(SCHEMA)}.fragment_links SET to_id = $1
          ...
          await queryWithAgentVector("system",
            `UPDATE ${escapeId(SCHEMA)}.fragments SET valid_to = NOW() WHERE id = $1 AND valid_to IS NULL`,
            [oldId], "write"
          );

          await queryWithAgentVector("system",
            `UPDATE ${escapeId(SCHEMA)}.fragments SET access_count = $1 WHERE id = $2`,
