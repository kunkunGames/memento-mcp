...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** 중복 제거 */
    const unique = this._deduplicate(combined, sq.fragmentCount, sq.anchorTime, sq._profile);
  ...
  async _buildTextRRF(sq, l1Ids, l1MissIds, l1IsFallback, cached, agentId, keyId, timeRange, searchPath, layerLatency) {
    ...
    const rrfLayers = [
      ...
      { name: "l2",    results: [...cached, ...l2Results], weightFactor: sq._profile?.l2WeightFactor ?? 1.0 },
