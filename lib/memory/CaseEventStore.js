...
export class CaseEventStore {
    ...
    try {
      ...
      const seqResult = await client.query(
        `SELECT COALESCE(MAX(sequence_no), -1) + 1 AS next_seq
           FROM ${escapeId(SCHEMA)}.case_events
          WHERE case_id = $1`,
        [event.case_id]
      );
  ...
  async addEdge(fromEventId, toEventId, edgeType, confidence = 1.0) {
    ...
    await pool.query(
      `INSERT INTO ${escapeId(SCHEMA)}.case_event_edges
  ...
  async addEvidence(fragmentId, eventId, kind, confidence = 1.0) {
    ...
    await pool.query(
      `INSERT INTO ${escapeId(SCHEMA)}.fragment_evidence
  ...
  async getEdgesByEvents(eventIds, keyId = null) {
    ...
    if (keyId != null) {
      ...
      const { rows } = await pool.query(
        `SELECT ee.from_event_id, ee.to_event_id, ee.edge_type, ee.confidence
           FROM ${escapeId(SCHEMA)}.case_event_edges ee
           JOIN ${escapeId(SCHEMA)}.case_events ce_from ON ce_from.event_id = ee.from_event_id
           JOIN ${escapeId(SCHEMA)}.case_events ce_to   ON ce_to.event_id   = ee.to_event_id
    ...
    const { rows } = await pool.query(
      `SELECT from_event_id, to_event_id, edge_type, confidence
         FROM ${escapeId(SCHEMA)}.case_event_edges
  ...
  async getEvidenceByEvent(eventId, keyId = null) {
    ...
    const { rows } = await pool.query(
      `SELECT f.id AS fragment_id, f.content, f.type, f.topic, f.keywords,
         ...
         FROM ${escapeId(SCHEMA)}.fragment_evidence fe
         JOIN ${escapeId(SCHEMA)}.fragments f ON f.id = fe.fragment_id
        WHERE fe.event_id = $1
          ${keyClause}
  ...
  async getEventsByFragment(fragmentId, keyId = null) {
    ...
    const { rows } = await pool.query(
      `SELECT ce.event_id, ce.case_id, ce.event_type, ce.summary, ce.created_at,
              fe.kind, fe.confidence
         FROM ${escapeId(SCHEMA)}.fragment_evidence fe
         JOIN ${escapeId(SCHEMA)}.case_events ce ON ce.event_id = fe.event_id
  ...
  async deleteExpired() {
    ...
    const result = await pool.query(
      `DELETE FROM ${escapeId(SCHEMA)}.case_events
