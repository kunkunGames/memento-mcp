...
export class ContradictionDetector {
  ...
  async detectSupersessions() {
    ...
    for (const pair of candidates.rows) {
      try {
        ...
        if (verdict.supersedes) {
          ...
          logInfo(`[ContradictionDetector] Supersession detected: ${older.id} -> ${newer.id}: ${verdict.reasoning}`);
          detected++;
        }
      } catch (err) {
    ...
    if (supersessionPairs.length > 0) {
      // createLinks requires pairs to be sorted by (min(from,to), max(from,to)) for deadlock prevention
      const sortedPairs = [...supersessionPairs].sort((a, b) => {
        const keyA = a.fromId < a.toId ? `${a.fromId}_${a.toId}` : `${a.toId}_${a.fromId}`;
        const keyB = b.fromId < b.toId ? `${b.fromId}_${b.toId}` : `${b.toId}_${b.fromId}`;
        return keyA.localeCompare(keyB);
      });

      await this.store.createLinks(sortedPairs, "system");
      await queryWithAgentVector("system",
