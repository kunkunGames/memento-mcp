    ...
    for (const newFrag of newFrags.rows) {
      const candidates = await queryWithAgentVector("system",
        `SELECT c.id, c.content, c.topic, c.type, c.importance,
                ...
                1 - (c.embedding <=> (SELECT embedding FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1)) AS similarity
         FROM ${escapeId(SCHEMA)}.fragments c
         WHERE c.id != $1
           AND c.topic = $2
           AND c.embedding IS NOT NULL
           AND c.key_id IS NOT DISTINCT FROM $3
           AND 1 - (c.embedding <=> (SELECT embedding FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1)) > 0.85
           AND NOT EXISTS (
    ...
    if (newDate > oldDate) {
      if (!candidate.is_anchor) {
        await queryWithAgentVector("system",
          `UPDATE ${escapeId(SCHEMA)}.fragments SET importance = importance * 0.5 WHERE id = $1`,
          [candidate.id], "write"
        );
      }
  ...
  async detectSupersessions() {
    ...
    const candidates = await queryWithAgentVector("system",
      `SELECT a.id AS id_a, a.content AS content_a, a.created_at AS created_a,
       ...
       FROM ${escapeId(SCHEMA)}.fragments a
       JOIN ${escapeId(SCHEMA)}.fragments b ON a.topic = b.topic
                                  AND a.type = b.type
                                  AND a.id < b.id
      ...
      try {
        ...
        if (verdict.contradicts) {
          const fragAResult = await queryWithAgentVector("system",
            `SELECT id, content, created_at, is_anchor, key_id FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1`,
            [entry.idA]
          );
