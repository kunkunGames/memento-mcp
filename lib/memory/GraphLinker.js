...
export class GraphLinker {
   ...
   * @param {number|null} keyId        - DB API key ID (null = master key, 전체 접근)
   * @param {number[]}    groupKeyIds  - 키 그룹 공유 대상 ID 배열 (옵션)
   * @returns {number} 생성된 링크 수
   */
  async linkFragment(fragmentId, agentId = "default", keyId = null, groupKeyIds = []) {
    /** 허용 key_id 집합 결정: master(null)는 모든 파편 접근, API key는 자신+그룹만 */
    const allowedKeyIds = keyId != null
      ? [keyId, ...(Array.isArray(groupKeyIds) ? groupKeyIds : [])]
      : null;

    /** key_id 격리 조건 헬퍼 — null(master)이면 조건 없음 */
    const keyFilter = (alias = "") => {
    ...
    const fragResult = await queryWithAgentVector(agentId,
      `SELECT id, content, topic, type, created_at
       FROM ${escapeId(SCHEMA)}.fragments
    ...
    const dedupResult = await queryWithAgentVector(agentId,
      `SELECT id,
              1 - (embedding <=> (SELECT embedding FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1)) AS similarity
       FROM ${escapeId(SCHEMA)}.fragments
       WHERE id != $1
         AND topic = $2
         AND embedding IS NOT NULL
         AND valid_to IS NULL
         AND 1 - (embedding <=> (SELECT embedding FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1)) >= 0.90
         ${keyFilter()}
    ...
    if (dedupResult.rows && dedupResult.rows.length > 0) {
      ...
      if (similarity >= 0.95) {
        ...
        await queryWithAgentVector(agentId,
          `UPDATE ${escapeId(SCHEMA)}.fragments SET valid_to = NOW()
           WHERE id = $1 AND valid_to IS NULL`,
          [fragmentId], "write"
        );
        await queryWithAgentVector(agentId,
          `UPDATE ${escapeId(SCHEMA)}.fragments SET access_count = access_count + 1
           WHERE id = $1`,
          [existing.id], "write"
        );
    ...
    const candidates = await queryWithAgentVector(agentId,
      `SELECT id, content, type, created_at, is_anchor,
       ...
       WHERE id != $1
         AND topic = $2
         AND embedding IS NOT NULL
         AND 1 - (embedding <=> (SELECT embedding FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1)) > 0.7
         ${keyFilter()}
       ORDER BY similarity DESC
       LIMIT 3`,
      [fragmentId, newFragment.topic]
  ...
  async retroLink(batchSize = 20) {
    const isolated = await queryWithAgentVector("system",
      `SELECT f.id
       FROM ${escapeId(SCHEMA)}.fragments f
       WHERE f.embedding IS NOT NULL
         AND (f.linked_to IS NULL OR array_length(f.linked_to, 1) IS NULL)
         AND NOT EXISTS (
           SELECT 1 FROM ${escapeId(SCHEMA)}.fragment_links l
  ...
  async buildCoRetrievalLinks(fragmentIds, sessionId, agentId = "default") {
    ...
    for (let i = 0; i < ids.length; i++) {
      for (let j = i + 1; j < ids.length; j++) {
        try {
          await queryWithAgentVector(agentId,
            `INSERT INTO ${escapeId(SCHEMA)}.fragment_links (from_id, to_id, relation_type, weight)
             VALUES ($1, $2, 'co_retrieved', 1)
             ON CONFLICT (from_id, to_id) DO UPDATE
               SET weight = ${escapeId(SCHEMA)}.fragment_links.weight + 1`,
            [ids[i], ids[j]], "write"
          );
          count++;
        } catch { /* 중복, FK 오류 등 무시 */ }
