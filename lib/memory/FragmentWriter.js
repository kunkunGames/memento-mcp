...
import { buildSearchPath, escapeId }             from "../config.js";
...
export class FragmentWriter {
  ...
  async ensureSchema() {
    ...
    try {
      ...
      await queryWithAgentVector("default", `CREATE SCHEMA IF NOT EXISTS ${escapeId(SCHEMA)}`, [], "write");
  ...
  async insert(fragment, { client: externalClient } = {}) {
     ...
     */
    const table = `${escapeId(SCHEMA)}.fragments`;
    if (externalClient) {
    ...
    const insertSql = `INSERT INTO ${table}
             ...
             ${onConflictClause}
                importance  = GREATEST(${table}.importance, EXCLUDED.importance),
                is_anchor   = ${table}.is_anchor OR EXCLUDED.is_anchor,
                accessed_at = NOW()
             RETURNING id`;
  ...
  async archiveVersion(fragment, agentId = "default") {
    await queryWithAgentVector(agentId,
      `INSERT INTO ${escapeId(SCHEMA)}.fragment_versions
    ...
    if (!existing) {
      const lookup = await queryWithAgentVector(agentId,
        `SELECT id, content, topic, keywords, type, importance,
         ...
         FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1`,
        [id]
      );
    ...
    try {
      ...
      if (updates.content !== undefined) {
        ...
        const dup = await client.query(
          `SELECT id FROM ${escapeId(SCHEMA)}.fragments
           WHERE content_hash = $1
             AND id != $2
      ...
      const result = await client.query(
        `UPDATE ${escapeId(SCHEMA)}.fragments
               SET ${setClauses.join(", ")}
               WHERE id = $1
  ...
  async delete(id, agentId = "default", keyId = null) {
    ...
    if (keyId) {
      const ownership = await queryWithAgentVector(agentId,
        `SELECT id FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1 AND key_id = $2`,
        [id, keyId]
      );
    ...
    await queryWithAgentVector(agentId,
      `DELETE FROM ${escapeId(SCHEMA)}.fragment_links
    ...
    const result = await queryWithAgentVector(agentId,
      `DELETE FROM ${escapeId(SCHEMA)}.fragments WHERE id = $1`,
      [id],
      "write"
    );
  ...
  async deleteMany(ids, agentId = "default", keyId = null) {
    ...
    let sql    = `DELETE FROM ${escapeId(SCHEMA)}.fragments WHERE id = ANY($1)`;
    const args = [ids];
    if (keyId) {
      sql += ` AND key_id = $2`;
      args.push(keyId);
  ...
  async deleteByAgent(agentId) {
    ...
    const result = await queryWithAgentVector(agentId,
      `DELETE FROM ${escapeId(SCHEMA)}.fragments WHERE agent_id = $1`,
      [agentId],
      "write"
    );
       ...
       WHERE id IN (
         SELECT DISTINCT
           CASE WHEN fl.from_id = ANY($1::text[]) THEN fl.to_id
                ELSE fl.from_id
           END
         FROM ${escapeId(SCHEMA)}.fragment_links fl
  ...
  async updateCaseId(id, caseId, keyId = null) {
    ...
    const result = await queryWithAgentVector("system",
      `UPDATE ${escapeId(SCHEMA)}.fragments SET case_id = $2
  ...
  async updateTtlTier(id, ttlTier, keyId = null) {
    let sql    = `UPDATE ${escapeId(SCHEMA)}.fragments SET ttl_tier = $2 WHERE id = $1`;
  ...
  async deleteExpired() {
    ...
    const result = await queryWithAgentVector("system",
      `WITH gc_candidates AS (
         ...
         WHERE ttl_tier NOT IN ('permanent')
           ...
           AND (
             ...
             (type IN ('fact', 'decision')
              ...
              AND NOT EXISTS (
                SELECT 1 FROM ${escapeId(SCHEMA)}.fragment_links fl
       ...
       DELETE FROM ${escapeId(SCHEMA)}.fragments WHERE id IN (SELECT id FROM gc_candidates)`,
