...
export class RecallSuggestionEngine {
  ...
  async _getRecentEvents(keyId, since) {
    ...
    const sql = keyVal === null
      ? `SELECT query_type, filter_keys, result_count
           FROM ${escapeId(SCHEMA)}.search_events
  ...
  async _getFragmentCount(keyId) {
    ...
    const { rows } = keyVal === null
      ? await pool.query(
          `SELECT COUNT(*)::int AS cnt FROM ${escapeId(SCHEMA)}.fragments WHERE key_id IS NULL AND valid_to IS NULL`
        )
      : await pool.query(
          `SELECT COUNT(*)::int AS cnt FROM ${escapeId(SCHEMA)}.fragments WHERE key_id = $1 AND valid_to IS NULL`,
          [keyVal]
        );
