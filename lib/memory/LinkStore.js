...
export class LinkStore {
        ...
        `INSERT INTO ${escapeId(SCHEMA)}.fragment_links (from_id, to_id, relation_type, weight)
               ...
               ON CONFLICT (from_id, to_id) DO UPDATE
                 SET relation_type = EXCLUDED.relation_type,
                     weight        = GREATEST(${escapeId(SCHEMA)}.fragment_links.weight, EXCLUDED.weight)`,
  ...
  async createLinks(pairs, agentId = "default") {
    ...
    try {
      ...
      await queryWithAgentVector(agentId,
        `UPDATE ${escapeId(SCHEMA)}.fragments f
         SET linked_to = (
      ...
      result = await queryWithAgentVector(agentId,
        `SELECT DISTINCT ON (id)
         ...
         FROM (
           ...
           FROM ${escapeId(SCHEMA)}.fragment_links l
           JOIN ${escapeId(SCHEMA)}.fragments f ON l.to_id = f.id
           ...
           SELECT f.id, f.content, f.topic, f.keywords, f.type,
                  ...
                  END AS relation_order
           FROM ${escapeId(SCHEMA)}.fragment_links l
           JOIN ${escapeId(SCHEMA)}.fragments f ON l.from_id = f.id
           WHERE l.to_id = ANY($1)
             AND l.relation_type = $2
             ${keyFilter}
         ) sub
  ...
  async getLinkedIds(fragmentId, agentId = "default", keyId = null) {
    ...
    const result = await queryWithAgentVector(agentId,
      `SELECT linked_to FROM ${escapeId(SCHEMA)}.fragments
       WHERE id = $1
  ...
  async isReachable(startId, targetId, agentId = "default", keyId = null) {
    ...
    const result = await queryWithAgentVector(agentId,
      `WITH RECURSIVE reachable AS (
         ...
         FROM ${escapeId(SCHEMA)}.fragments f0
         WHERE f0.id = $1
           AND f0.valid_to IS NULL
       UNION
         SELECT unnest(f.linked_to), r.depth + 1
         FROM reachable r
         JOIN ${escapeId(SCHEMA)}.fragments f ON f.id = r.id
         WHERE r.depth < 20
           AND r.id != $2
  ...
  async getRCAChain(startId, agentId = "default", keyId = null) {
    ...
    const result = await queryWithAgentVector(agentId,
      `WITH rca AS (
         ...
         FROM ${escapeId(SCHEMA)}.fragment_links l
         JOIN ${escapeId(SCHEMA)}.fragments f2 ON l.to_id = f2.id
         WHERE l.from_id = $1
